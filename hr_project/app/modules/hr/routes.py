...
from app.models import (
    Employee, DictionaryItem, Order, EmploymentRecord, LeaveCategory, BildirisRecord,
    LeaveReason, Holiday, LeaveRequest, VacationCompensation,
    InsurancePolicy, SalaryCard, Document,
)
...
from app.services.hr_service import recompute_employee_from_history, recompute_employee_contract_from_bildiris
from app.services.leave_service import (
    compute_leave_periods, get_leave_balance, compute_end_date, validate_leave_request,
)
...
def _dict_options(category):
    return (DictionaryItem.query
            .filter_by(module_code=MODULE, category=category, is_active=True)
...
def add_order():
    ...
    return render_form("hr/order_form.html", order=None, order_types=order_types, usage_records=[], usage_notifications=[])
...
def edit_order(order_id):
    ...
    order_types = _dict_options("order_type")
    usage_records = (EmploymentRecord.query.filter_by(order_id=order.id)
                      .order_by(EmploymentRecord.date_from.desc()).all())
    usage_notifications = (BildirisRecord.query.filter_by(order_id=order.id)
                            .order_by(BildirisRecord.start_date.desc()).all())
    if request.method == "POST":
        ...
        return modal_redirect("hr.list_orders")
    return render_form("hr/order_form.html", order=order, order_types=order_types,
                        usage_records=usage_records, usage_notifications=usage_notifications)
...
def delete_order(order_id):
    order = Order.query.get_or_404(order_id)
    in_use = (EmploymentRecord.query.filter_by(order_id=order.id).first()
              or BildirisRecord.query.filter_by(order_id=order.id).first())
    if in_use:
        if is_modal_request():
            return jsonify({"success": False, "html": "<div class='flash flash-danger'>Bu əmr iş yerləri və ya bildiriş qeydlərində istifadə olunur, silinə bilməz.</div>"})
        flash("Bu əmr iş yerləri və ya bildiriş qeydlərində istifadə olunur, silinə bilməz.", "danger")
...
def api_work_history(emp_id):
    records = (EmploymentRecord.query.filter_by(employee_id=emp_id)
               .order_by(EmploymentRecord.date_from.desc()).all())
    data = [{
        "id": r.id,
        "is_current_company": r.is_current_company,
        "ticket": "Cari şirkət" if r.is_current_company else "Kənar iş yeri",
        "movement_type": MOVEMENT_LABELS.get(r.movement_type, r.movement_type) if r.is_current_company else None,
...
def add_work_history(emp_id):
    ...
    if request.method == "POST":
        ...
        if error:
            flash(error, "danger")
            return render_form("hr/work_history_form.html", employee=employee, record=None,
...
def edit_work_history(emp_id, record_id):
    employee = Employee.query.get_or_404(emp_id)
    record = EmploymentRecord.query.filter_by(id=record_id, employee_id=emp_id).first_or_404()
    if request.method == "POST":
        error = _validate_work_history_form(request.form, employee_id=emp_id, exclude_id=record.id)
        if error:
            flash(error, "danger")
            return render_form("hr/work_history_form.html", employee=employee, record=record,
                                **_work_history_form_choices())
        _apply_work_history_form(record, request.form)
        recompute_employee_from_history(employee)
        db.session.commit()
...
def _validate_work_history_form(form, employee_id, exclude_id=None):
    ...
    if conflict:
        ...
        conflict_end = conflict.date_to.isoformat() if conflict.date_to else "davam edir"
        return (f"Tarix aralığı mövcud {scope} qeydi ilə kəsişir "
...
def _apply_leave_category_form(category, form):
    ...
    category.seniority_years_per_bonus = _parse_int(form.get("seniority_years_per_bonus")) or 0
    category.bonus_days_per_interval = _parse_int(form.get("bonus_days_per_interval")) or 0
...
def delete_leave_category(category_id):
    category = LeaveCategory.query.get_or_404(category_id)
    if BildirisRecord.query.filter_by(leave_category_id=category.id).first():
        if is_modal_request():
            return jsonify({"success": False, "html": "<div class='flash flash-danger'>Bu kateqoriya bildiriş qeydlərində istifadə olunur, silinə bilməz.</div>"})
        flash("Bu kateqoriya bildiriş qeydlərində istifadə olunur, silinə bilməz.", "danger")
...
# ---------------------------------------------------------------------------
# Bildirişlər (BildirisRecord) — DSMF saytına daxil edilmiş məlumatların
...
def _notifications_form_choices():
    return {
        ...
        "leave_categories": LeaveCategory.query.filter_by(is_active=True).order_by(LeaveCategory.name).all(),
        "orders": Order.query.order_by(Order.order_date.desc()).all(),
    }
...
def api_notifications(emp_id):
    records = (BildirisRecord.query.filter_by(employee_id=emp_id)
               .order_by(BildirisRecord.start_date.desc(), BildirisRecord.created_at.desc()).all())
    data = [{
        "id": r.id,
        "number": r.number,
        "start_date": r.start_date.isoformat() if r.start_date else None,
        "order": r.order.label() if r.order else None,
        "employment_classification": r.employment_classification.name if r.employment_classification else None,
        "employment_position": r.employment_position.name if r.employment_position else None,
...
def _apply_notification_form(record, form):
    if not record.id and record.employee_id:
        previous = (BildirisRecord.query.filter_by(employee_id=record.employee_id)
                    .order_by(BildirisRecord.start_date.desc(), BildirisRecord.created_at.desc()).first())
        if previous:
            for field in ["employment_classification_id","employment_position_id","contract_number","contract_type_id","contract_start_date","contract_end_date","work_type_id","labor_type_id","leave_category_id","salary","note"]:
    ...
    record.employment_classification_id = _parse_int(form.get("employment_classification_id"))
...
def add_notification(emp_id):
    ...
    latest = (BildirisRecord.query.filter_by(employee_id=emp_id)
              .order_by(BildirisRecord.start_date.desc(), BildirisRecord.created_at.desc()).first())
    if request.method == "POST":
        error = _validate_notification_form(request.form)
        if error:
            flash(error, "danger")
            return render_form("hr/notification_form.html", employee=employee, record=None,
                                latest=latest, **_notifications_form_choices())
        record = BildirisRecord(employee_id=employee.id)
...
@hr_bp.route("/<int:emp_id>/notifications/edit/<int:record_id>", methods=["GET", "POST"])
@login_required
@permission_required(MODULE, "can_edit")
@log_action(MODULE, "EDIT_NOTIFICATION")
def edit_notification(emp_id, record_id):
    employee = Employee.query.get_or_404(emp_id)
    record = BildirisRecord.query.filter_by(id=record_id, employee_id=emp_id).first_or_404()
    if request.method == "POST":
        error = _validate_notification_form(request.form)
        if error:
            flash(error, "danger")
            return render_form("hr/notification_form.html", employee=employee, record=record,
                                latest=None, **_notifications_form_choices())
        _apply_notification_form(record, request.form)
        recompute_employee_contract_from_bildiris(employee)
        db.session.commit()
...
def api_leave_reasons():
    ...
    data = [{
        "id": r.id, "name": r.name, "counting_method": r.counting_method_label(),
        "is_annual_leave": r.is_annual_leave, "is_active": r.is_active,
    } for r in items]
    return jsonify(data)
...
def add_leave_reason():
    ...
    return render_form("hr/leave_reason_form.html", reason=None, methods=LeaveReason.COUNTING_METHODS)
...
def edit_leave_reason(reason_id):
    ...
    if request.method == "POST":
        ...
        return modal_redirect("hr.list_leave_reasons")
    return render_form("hr/leave_reason_form.html", reason=reason, methods=LeaveReason.COUNTING_METHODS)
...
def delete_leave_reason(reason_id):
    ...
    if LeaveRequest.query.filter_by(leave_reason_id=reason.id).first():
        if is_modal_request():
            return jsonify({"success": False, "html": "<div class='flash flash-danger'>Bu səbəb iş buraxma qeydlərində istifadə olunur, silinə bilməz.</div>"})
        flash("Bu səbəb iş buraxma qeydlərində istifadə olunur, silinə bilməz.", "danger")
...
def api_holidays():
    ...
    return jsonify([{"id": h.id, "date": h.date.isoformat(), "name": h.name} for h in items])
...
def add_holiday():
    if request.method == "POST":
        holiday = Holiday(date=_parse_date(request.form.get("date")),
...
def set_compensation(emp_id):
    employee = Employee.query.get_or_404(emp_id)
    period_start = _parse_date(request.args.get("period_start") or request.form.get("period_start"))
    period_end = _parse_date(request.args.get("period_end") or request.form.get("period_end"))
    comp = VacationCompensation.query.filter_by(employee_id=emp_id, period_start=period_start).first()

    if request.method == "POST":
        if not comp:
            comp = VacationCompensation(employee_id=emp_id, period_start=period_start, period_end=period_end)
            db.session.add(comp)
        comp.period_end = period_end
        comp.compensated_base_days = _parse_int(request.form.get("compensated_base_days")) or 0
    ...
    return render_form("hr/compensation_form.html", employee=employee, comp=comp,
...
def api_leave_requests(emp_id):
    records = (LeaveRequest.query.filter_by(employee_id=emp_id)
...
def _leave_request_form_choices():
    return {
        "reasons": LeaveReason.query.filter_by(is_active=True).order_by(LeaveReason.name).all(),
        "orders": Order.query.order_by(Order.order_date.desc()).all(),
    }
    ...
    if request.method == "POST":
        ...
        if not reason or not start or not day_count:
            ...
            if overlap_error:
                ...
                error = (f"Kifayət qədər məzuniyyət günü yoxdur. Qalıq: "
                         f"{balance['base']} əsas + {balance['bonus']} əlavə = {balance['total']} gün.")
        if error:
            flash(error, "danger")
            return render_form("hr/leave_request_form.html", employee=employee, record=None,
                                balance=balance, **_leave_request_form_choices())
...
@hr_bp.route("/<int:emp_id>/leave-requests/edit/<int:record_id>", methods=["GET", "POST"])
@login_required
@permission_required(MODULE, "can_edit")
@log_action(MODULE, "EDIT_LEAVE_REQUEST")
def edit_leave_request(emp_id, record_id):
    employee = Employee.query.get_or_404(emp_id)
    record = LeaveRequest.query.filter_by(id=record_id, employee_id=emp_id).first_or_404()
    balance = get_leave_balance(employee)
    if request.method == "POST":
        reason = LeaveReason.query.get(_parse_int(request.form.get("leave_reason_id")))
        start = _parse_date(request.form.get("start_date"))
        ...
        if not reason or not start or not day_count:
            ...
            overlap_error = validate_leave_request(employee, start, end, exclude_id=record.id)
            if overlap_error:
                error = overlap_error
                ...
                if day_count > available:
                    error = (f"Kifayət qədər məzuniyyət günü yoxdur. Qalıq (bu qeyd nəzərə alınmadan): "
                             f"{available} gün.")
        if error:
            flash(error, "danger")
            return render_form("hr/leave_request_form.html", employee=employee, record=record,
                                balance=balance, **_leave_request_form_choices())
...
def api_insurance(emp_id):
    ...
    records = (InsurancePolicy.query.filter_by(employee_id=emp_id)
...
def _validate_insurance_form(form, employee_id, exclude_id=None):
    ...
    conflict = find_overlapping(existing, new_start, new_end, exclude_id=exclude_id,
                                 start_attr="start_date", end_attr="end_date")
    if conflict:
        return (f"Tarix aralığı mövcud sığorta qeydi ilə kəsişir "
...
def add_insurance(emp_id):
    ...
    if request.method == "POST":
        ...
        if error:
            flash(error, "danger")
            return render_form("hr/insurance_form.html", employee=employee, record=None, **_insurance_form_choices())
...
def edit_insurance(emp_id, record_id):
    employee = Employee.query.get_or_404(emp_id)
    record = InsurancePolicy.query.filter_by(id=record_id, employee_id=emp_id).first_or_404()
    if request.method == "POST":
        error = _validate_insurance_form(request.form, employee_id=emp_id, exclude_id=record.id)
        if error:
            flash(error, "danger")
            return render_form("hr/insurance_form.html", employee=employee, record=record, **_insurance_form_choices())
        _apply_insurance_form(record, request.form)
        db.session.commit()
...
def api_salary_cards(emp_id):
    today = datetime.utcnow().date()
    records = (SalaryCard.query.filter_by(employee_id=emp_id)
               .order_by(SalaryCard.valid_until.desc()).all())
    data = []
    for r in records:
        status = "active"
        if r.valid_until:
...
def add_salary_card(emp_id):
    ...
    return render_form("hr/salary_card_form.html", employee=employee, record=None,
...
def edit_salary_card(emp_id, record_id):
    ...
    if request.method == "POST":
        ...
        return modal_redirect("hr.salary_card_list", emp_id=employee.id)
    return render_form("hr/salary_card_form.html", employee=employee, record=record,
                        banks=_dict_options("bank"))
...
def api_documents(emp_id):
    records = (Document.query.filter_by(employee_id=emp_id)
...
def add_document(emp_id):
    ...
    if request.method == "POST":
        ...
        if not stored:
            ...
            return render_form("hr/document_form.html", employee=employee,
